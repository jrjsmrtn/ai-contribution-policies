...
* **Added [`projects/duckdb.md`](projects/duckdb.md)** — asked for, not previously surveyed, and absent
  from the third-party lead list. **DuckDB carries two AI policies that disagree.** `CONTRIBUTING.md` has
  said since 2025-06-11 *"Please do not submit pull requests generated by AI (LLMs)"*; `AI_POLICY.md`,
  added 2026-06-03 as *"Initial version that captures our current stance"*, opens *"Using AI (i.e., LLMs)
  as tools for coding is welcome"*. Both files are identical on `main` and the default branch, neither
  links the other, and both were merged the same day they were opened, by the same maintainer account,
  with no review.
* **The newer file is ripgrep's, with four sentences changed** — measured sentence by sentence with
  headings set aside: comments addressed to *"maintainers and contributors"*; comments *"hidden/closed"*
  where Astral and ripgrep only hide them; a clause that pasted AI output *"is not a substitute for human
  commentary"*; and a citation that credits both hops, ripgrep and uv. **It is the only borrower here that
  names two hops of its lineage.** It links uv at ripgrep's pinned commit, copied verbatim, and ripgrep at
  `master` — the pin travelled, the pinning did not. Astral's criticality argument is still absent.
* **Practice measured by clone, not search.** A partial clone of both branches counts 141 commits committed
  since 2026-05-01 with a `Co-Authored-By` trailer naming Anthropic, from fifteen author names, including
  the account that merged both policies; 33 were authored in April and May, while the prohibition was the
  only written rule. **GitHub's commit search reported 42**, and a control term 25 against 312 in the
  clone, so search is not usable for these counts.
* **`CLAUDE.md` became a `@AGENTS.md` import on 2026-06-05, "avoided symlinking because it complicates
  things on Windows"** — the only stated reason in this bundle for choosing an import over a symlink.
* **Knock-on updates.** [`policy-by-copying`](mechanisms/policy-by-copying.md) gains a seventh copying
  event and a branch in its lineage; [`human-in-the-loop`](mechanisms/human-in-the-loop.md) moves to ten
  records and four hedged prohibitions; [`prose-reserved-to-humans`](mechanisms/prose-reserved-to-humans.md)
  to eighteen, noting that DuckDB kept the prose clause systemd dropped;
  [`agent-file-pointers`](mechanisms/agent-file-pointers.md) records the Windows trade-off; `ripgrep`,
  `astral`, `nerves` and `networkmanager` follow.
